...
def download_pdf_from_iframe(driver, view_url, filename):
    try:
        ...
        if not pdf_url:
            return {"filename": filename, "status": "No iframe found"}
        ...
        if resp.status_code == 200:
            ...
            return {"filename": filename, "status": f"Download failed ({resp.status_code})"}
    except Exception as e:
        return {"filename": filename, "status": f"Error: {str(e)}"}
...
def download_all_supreme_court_pdfs():
    ...
    for item in judgment_links:
        result = download_pdf_from_iframe(driver, item['url'], item['filename'])
